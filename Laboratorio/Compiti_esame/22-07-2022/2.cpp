...
class Tank{
    ...
    double fill(double fuel){
        double eccesso_carburante = 0;
        if((fuel + fuelLevel) <= capacity){
            return 0;
        }else{
            fuelLevel = capacity;
            eccesso_carburante = fuelLevel + fuel;
            return eccesso_carburante;
        }
    }
...
class Vehicle{
    ...
    public:
    Vehicle(double wheels, double chilomters, double tankCapacity, double fuelLevel) : tank(tankCapacity, fuelLevel){
        this->wheels = wheels;
    ...
    virtual ostream& put(ostream& os){
        os << "Class= "<< typeid(*this).name() << ", wheels=" << wheels << ", chilomters=" << chilometers;
        os << ", tank.getCapacity()=" << tank.getCapacity() << ", tank.getFuelLevel()=" << tank.getTankLevel();
        return os;
    }
};
...
class Truck : public Vehicle{
    public:
    Truck(double chilomters) : Vehicle(6, chilometers, 110, 50) {}

    bool drive(double km){
        double carburante_necessario = km * 0.4;
        if(chilometers > 200000){
            carburante_necessario += ((carburante_necessario * 20)/100);
        }
