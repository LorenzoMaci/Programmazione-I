...
bool* esercizio31(double*** M, int n, int m, int a, int b){
    bool* A = new bool[2*m]; //array unidimensionale

    for(int i = 0; i<2*m; i++){
        A[i] = false; //elementi di indice dispari
    }
    double colonna;
    for(int i=0; i<m; i++){
        colonna= 0;
        for(int j=0; j<n; j++){
                if(*M[j][i] >= a && *M[j][i] <= b){
                    colonna += *M[j][i]; //somma elementi della colonna
                }
            }
           
        if(colonna/n > (a+b)/2){
            A[2*i] = true; //elementi veri dell'array unidimensionale
        }
    }
    cout << "Array di Bool:  "; //nuovo array
    for(int i=0; i<2*m; i++){
        cout << A[i]<< "   ";
    } 
    cout<<endl;
