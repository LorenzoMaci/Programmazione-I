...
int es1(double*** A, int m, int n, short k, double q){
    int sequenze = 0;
    for(int i=0; i<=m-n; i++){ //Per poter prendere l'elemento successivo delle righe
        for(int j=i; j<=n-k; j++){ //per prendere l'elemento sotto delle colonne
            if(A[i][j]){
                double somma_diagonale = 0.0;
                for(int p=j; p<j + k; p++){ 
                    if(A[p][p]){
                        somma_diagonale += *A[p][p]; //p  permette di prendere l'elemento successivo dell'altra diagonale
                    }
                }
